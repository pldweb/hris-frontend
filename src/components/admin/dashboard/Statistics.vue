  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
    ...
    >
      <div class="flex flex-col justify-center h-full relative z-10">
        ...
        <div class="flex items-center gap-2 mb-3">
          ...
          >
            ...
            <span class="text-brand-white text-xs font-semibold"
              >+{{ employees.added_this_month }} bulan ini</span
            >
          </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-3">
          <div>
            <p class="text-brand-white-90 text-sm font-medium">Pegawai Kami</p>
            <p
    ...
    >
      <div class="flex items-center justify-between">
        <div>
          <p class="text-brand-dark text-sm font-medium">Total Tim</p>
          <p class="text-brand-dark text-2xl sm:text-3xl font-extrabold leading-tight my-2">
            {{ loading ? '...' : teams.total }}
          </p>
          <p class="text-success text-sm font-medium">+{{ teams.new_teams }} tim baru</p>
        </div>
        <div
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p :class="attendance.change >= 0 ? 'text-success' : 'text-danger'" class="text-sm font-medium">
            {{ attendance.change >= 0 ? '+' : '' }}{{ attendance.change }}% dari minggu lalu
          </p>
        </div>
        <div
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p :class="tasks.change >= 0 ? 'text-success' : 'text-danger'" class="text-sm font-medium">
            {{ tasks.change >= 0 ? '+' : '' }}{{ tasks.change }} dari kemarin
          </p>
        </div>
        <div
    ...
    >
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-success text-sm font-medium">+{{ projects.new_projects }} proyek baru</p>
        </div>
        <div
