...
<template>
  <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="flex flex-col justify-center h-full relative z-10">
          ...
          <div class="flex items-center gap-2 mb-3">
            <div class="flex items-center gap-1 px-3 py-1 bg-white/20 rounded-full backdrop-blur-sm">
              ...
              <span class="text-brand-white text-xs font-semibold">+5.2% bulan ini</span>
            </div>
          </div>
          ...
          <div class="flex items-center gap-3 mt-auto">
            <div class="flex items-center gap-1">
              ...
              <span class="text-brand-white-70 text-xs font-normal">Semua Departemen</span>
            </div>
            <div class="flex items-center gap-1">
              <Star class="w-3 h-3 text-white opacity-70" />
              <span class="text-brand-white-70 text-xs font-normal">Tepat Waktu</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        <div class="flex items-center justify-between">
          <div class="flex-1 min-w-0 pr-2">
            <p class="text-brand-dark text-sm font-medium">Pegawai Dibayar</p>
      ...
      <div
        ...
        <div class="flex items-center justify-between">
          <div class="flex-1 min-w-0 pr-2">
            <p class="text-brand-dark text-sm font-medium">Pembayaran Tertunda</p>
            <p class="text-brand-dark text-3xl font-extrabold leading-tight my-2 truncate">
              {{ loading ? "..." : statistics.pending_review }}
            </p>
            <p class="text-danger text-sm font-medium">Butuh persetujuan</p>
          </div>
      ...
      <div
        ...
        <h3 class="text-brand-dark text-lg font-bold mb-4">Aksi Penggajian</h3>
        <div class="space-y-3">
          <RouterLink :to="{ name: 'admin.payroll.create' }"
            ...
            <span class="text-brand-white text-sm font-semibold">Buat Penggajian Baru</span>
          </RouterLink>
          ...
          <button
            ...
            <span class="text-brand-dark text-sm font-medium">Hasilkan Laporan</span>
          </button>

          <button
            class="btn-secondary w-full text-left border border-[#DCDEDD] rounded-[16px] hover:border-[#0C51D9] hover:border-2 hover:rounded-[12px] focus:border-[#0C51D9] focus:border-2 focus:rounded-[12px] focus:bg-white transition-all duration-300 px-4 py-3 flex items-center gap-2">
            <Download class="w-4 h-4 text-gray-600" />
            <span class="text-brand-dark text-sm font-medium">Ekspor Data</span>
          </button>

          <button
            class="btn-secondary w-full text-left border border-[#DCDEDD] rounded-[16px] hover:border-[#0C51D9] hover:border-2 hover:rounded-[12px] focus:border-[#0C51D9] focus:border-2 focus:rounded-[12px] focus:bg-white transition-all duration-300 px-4 py-3 flex items-center gap-2">
            <Settings class="w-4 h-4 text-gray-600" />
            <span class="text-brand-dark text-sm font-medium">Pengaturan Penggajian</span>
          </button>
        </div>
      </div>
      ...
      <div
        ...
        <div class="flex items-center justify-between">
          <div class="flex-1 min-w-0 pr-2">
            <p class="text-brand-dark text-sm font-medium">Rata-rata Gaji</p>
            ...
            <p class="text-success text-sm font-medium">
              +{{ formatRupiahCompact(1900000) }} dari bulan lalu
            </p>
          </div>
      ...
      <div
        ...
        <div class="flex items-center justify-between">
          <div class="flex-1 min-w-0 pr-2">
            <p class="text-brand-dark text-sm font-medium">Selesai</p>
            <p class="text-brand-dark text-3xl font-extrabold leading-tight my-2 truncate">
              {{ loading ? "..." : statistics.finalized }}
            </p>
            <p class="text-purple-600 text-sm font-medium">Bulan ini</p>
          </div>
    ...
    <div class="bg-white border border-[#DCDEDD] rounded-[20px] p-5">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-brand-dark text-lg font-bold">Periode Penggajian Bulanan</h3>
      </div>
      <div class="space-y-4">
        <div v-for="payroll in payrolls" :key="payroll.id"
          ...
          <div class="flex-1">
            ...
            <div class="flex items-center gap-2">
              ...
              <p class="text-brand-dark text-sm font-normal">
                {{ payroll.employee_count || 0 }} pegawai • Semua departemen
              </p>
            </div>
            <p class="text-brand-light text-xs font-normal mt-1">
              Diproses pada {{ formatProcessedDate(payroll.created_at) }}
            </p>
          </div>
          ...
          <div class="flex-1 flex flex-col justify-center items-center">
            <div class="text-left">
              ...
              <p class="text-brand-light text-sm font-normal">Total penggajian</p>
            </div>
          </div>
          <button @click="viewDetails(payroll.id)"
            ...
            <span class="text-brand-dark text-base font-medium">Detail</span>
          </button>
        </div>

        <div v-if="!loading && payrolls.length === 0" class="text-center py-12 text-gray-500">
          <p class="text-lg font-semibold">Tidak ada data penggajian ditemukan</p>
          <p class="text-sm">Buat penggajian pertama Anda untuk memulai</p>
        </div>
      </div>
    </div>
  </div>
