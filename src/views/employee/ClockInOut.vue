...
const locationName = ref("Lokasi tidak terdeteksi");
const locationAddress = ref("Klik 'Dapatkan Lokasi' untuk mendeteksi posisi Anda saat ini");
...
const attendanceStatus = computed(() => {
  if (!todayAttendance.value) {
    return { text: "Belum Absen Masuk", class: "bg-red-100 text-red-700" };
  }
  if (todayAttendance.value.check_in && !todayAttendance.value.check_out) {
    return { text: "Sudah Absen Masuk", class: "bg-green-100 text-green-700" };
  }
  if (todayAttendance.value.check_in && todayAttendance.value.check_out) {
    return { text: "Selesai", class: "bg-blue-100 text-blue-700" };
  }
  return { text: "Belum Absen Masuk", class: "bg-red-100 text-red-700" };
});
...
const getCurrentLocation = async () => {
  if (!navigator.geolocation) {
    updateLocationDisplay(
      ...
      "Browser Anda tidak mendukung layanan lokasi"
    );
    return;
  }
  ...
  updateLocationDisplay(
    "loading",
    "Mendapatkan lokasi...",
    "Harap tunggu selagi kami mendeteksi posisi Anda"
  );
    ...
    (error) => {
      ...
      switch (error.code) {
        case error.PERMISSION_DENIED:
          errorMessage = "Akses lokasi ditolak. Harap aktifkan izin lokasi.";
          break;
      ...
      updateLocationDisplay("error", "Gagal mendapatkan lokasi", errorMessage);
      isGettingLocation.value = false;
    },
    options
  );
...
const handleCheckIn = async () => {
  if (!currentLocation.value) {
    alert("Harap dapatkan lokasi Anda saat ini terlebih dahulu!");
    return;
  }
  ...
  try {
    ...
    alert("Gagal absen masuk. Silakan coba lagi.");
  }
};
...
const handleCheckOut = async () => {
  ...
  try {
    ...
    alert("Gagal absen keluar. Silakan coba lagi.");
  }
};
...
const resetForNextAction = () => {
  ...
  updateLocationDisplay(
    ...
    "Harap dapatkan lokasi lagi untuk tindakan absensi berikutnya"
  );
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
      ...
      >
        <div class="mb-6">
          <div class="flex items-center justify-center gap-3 mb-4">
            ...
            <div>
              <h3 class="text-brand-dark text-2xl font-bold">Waktu Saat Ini</h3>
      ...
      >
        <div class="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 class="text-brand-dark text-xl font-bold">Status Hari Ini</h3>
            <p class="text-brand-light text-sm">Absensi Anda hari ini</p>
          </div>
        </div>
        ...
        <div class="space-y-4">
          ...
          <div class="p-4 bg-gray-50 rounded-[12px] border border-[#DCDEDD]">
            <div class="flex items-center justify-between">
              <span class="text-brand-dark text-base font-medium">Masuk</span>
          ...
          <div class="p-4 bg-gray-50 rounded-[12px] border border-[#DCDEDD]">
            <div class="flex items-center justify-between">
              <span class="text-brand-dark text-base font-medium">Keluar</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
      ...
      >
        <div class="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 class="text-brand-dark text-xl font-bold">Lokasi & Foto</h3>
            <p class="text-brand-light text-sm">Hasilkan lokasi kerja Anda</p>
          </div>
        </div>
        ...
        <div class="mb-6">
          ...
          >
            ...
            <span class="text-brand-dark text-sm font-semibold">
              {{ isGettingLocation ? "Mendapatkan Lokasi..." : currentLocation ? "Perbarui Lokasi" : "Dapatkan Lokasi Saat Ini" }}
            </span>
          </button>
        ...
        <div class="space-y-4">
            ...
            >
              <div class="text-center">
                ...
                <p class="text-gray-500 text-sm">
                  Pratinjau kamera akan muncul di sini
                </p>
              </div>
            </div>
          ...
          <div v-if="!capturedPhotoData" class="flex gap-3">
            ...
            >
              ...
              <span class="text-white text-sm font-semibold">Ambil Foto</span>
            </button>
          </div>
      ...
      >
        <div class="mb-6">
          ...
          <h3 class="text-brand-dark text-2xl font-bold mb-2">
            {{ isCheckedIn ? "Siap untuk Keluar?" : "Siap untuk Masuk?" }}
          </h3>
          <p class="text-brand-light text-base">
            {{
              isCheckedIn
                ? "Tandai kepulangan Anda dengan lokasi"
                : "Tandai kehadiran Anda dengan lokasi"
            }}
          </p>
        </div>
        ...
        <p class="text-brand-light text-sm mt-4">
          ...
          Harap dapatkan lokasi Anda sebelum masuk/keluar
        </p>
      </div>
    </div>
  </div>
