...
const activeMembersStatus = computed(() => {
  ...
  if (active >= expected) return "Kapasitas penuh";
  if (active >= expected * 0.8) return "Hampir penuh";
  return "Berkembang";
});
    ...
    <main class="main-content flex-1 overflow-auto p-5">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        >
          <div class="flex items-center justify-between">
            <div>
              <p class="text-brand-dark text-base font-medium">Ukuran yang Diharapkan</p>
              <p
                class="text-brand-dark text-3xl font-extrabold leading-tight my-2"
              >
                {{ team?.expected_size || 0 }}
              </p>
              <p class="text-success text-base font-medium">Kapasitas tim</p>
            </div>
            <div
        ...
        >
          <div class="flex items-center justify-between">
            <div>
              <p class="text-brand-dark text-base font-medium">Ketua Tim</p>
              <p
                class="text-brand-dark text-xl font-extrabold leading-tight my-2"
              >
                {{ team?.leader?.name || "N/A" }}
              </p>
              <p class="text-success text-base font-medium">Ketua</p>
            </div>
            <div
      ...
      >
        ...
        <div v-if="loadingMembers" class="text-center py-12">
          <p class="text-gray-500 text-lg font-medium">Memuat anggota...</p>
        </div>
        <div
          v-else
          ...
          >
            ...
            <div class="space-y-1 mb-3">
              <div class="flex items-center gap-2 text-sm text-gray-500">
                <Calendar class="w-4 h-4" />
                <span>{{ member.joined_at ? `Bergabung ${formatDateUtil(member.joined_at)}` : "N/A" }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm text-gray-500">
                <Clock class="w-4 h-4" />
                <span
        ...
        >
          ...
          <p class="text-gray-500 text-lg font-medium">Tidak ada anggota tim</p>
          <p class="text-gray-400 text-sm">Anggota tim akan muncul di sini</p>
        </div>
      </div>
      ...
      >
        <div class="flex items-center justify-between mb-6">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-brand-dark text-lg font-bold">Proyek Tim</h3>
              <p class="text-brand-light text-sm">
                Proyek yang ditugaskan ke {{ team?.name }}
              </p>
            </div>
          </div>
          <div class="flex items-center gap-2">
        ...
        <div v-if="loadingProjects" class="text-center py-12">
          <p class="text-gray-500 text-lg font-medium">Memuat proyek...</p>
        </div>
        <div
          v-else
          class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4"
        ...
        >
          ...
          <p class="text-gray-500 text-lg font-medium">Tidak ada proyek yang ditugaskan</p>
          <p class="text-gray-400 text-sm">
            Proyek yang ditugaskan ke tim Anda akan muncul di sini
          </p>
        </div>
      </div>
      ...
      >
        <div class="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 class="text-brand-dark text-lg font-bold">Deskripsi Tim</h3>
            <p class="text-brand-light text-sm">Gambaran umum dan detail tim</p>
          </div>
        </div>
        ...
        <div class="space-y-4">
          <div>
            ...
            <p class="text-gray-600 text-base leading-relaxed">
              {{ team?.description || "Tidak ada deskripsi yang tersedia" }}
            </p>
          </div>
